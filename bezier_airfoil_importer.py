...
class Foil:
    def Execute(self, sel0, sel1, endleiste_soll):
        ...
        def get_profile(filename):
            ...
            try:
                if 10 != len(oben) or 10 != len(unten):
                    ...
                    ui.messageBox(f'Splinedegree of the file does not match 9 (detected: top {len(oben)} points, bottom{len(unten)} points instead of 10')
        ...
        # sketch curves
        curve = sketchTest.sketchCurves.sketchControlPointSplines.add(controlPoints, 3)
        ...
        # find out the coordinates of the controlPoints
        for p in range(len(curve.controlPoints)):

            point = curve.controlPoints[p]
            punktereihe.append((point.geometry.x, point.geometry.y, point.geometry.z, p))

        for p in range(len(curve2.controlPoints)):

            point = curve2.controlPoints[p]
            punktereihe2.append((point.geometry.x, point.geometry.y, point.geometry.z, p))
        ...
        for p in range(len(curve.controlPoints)):  #0
            ...
            # avoid the origin because it creates constraints that will screw up the curves
            # so move to x + 1 and y +1 one first, add the data from the bez.dat and than subtract the detected values then go to startingpoint of the line
            point.move(vector11)            
            point.move(vector_m)
            point.move(vector_p)
            point.move(to_orig)
        ...
        aas = []
        # same stuff for the bottom, should be put in a funktion ...
        for p in range(len(curve2.controlPoints)):
            point = curve2.controlPoints[p]
            a = point.geometry.asArray()
            aas.append((a[0], a[1], a[2]))
            v = adsk.core.Vector3D.create(-1 * a[0], -1 * a[1], -1 * a[2])
            vector_m = adsk.core.Vector3D.create(bezunten[p][0] * wurzeltiefe, bezunten[p][1] * wurzeltiefe, 0.0)  


            if 0 <= p < len(curve2.controlPoints):
                point.move(vector11)
                point.move(vector_m)
                point.move(v)
                point.move(to_orig)
...
class FoilCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args: adsk.core.CommandEventArgs):
        try:
            ...
            i2.addSelectionFilter(adsk.core.SelectionCommandInput.SketchLines)
            i3 = inputs.addValueInput(IN01_INPUT1_COMMAND_ID, IN01_INPUT1_COMMAND_ID, "mm", adsk.core.ValueInput.createByReal(0.0))
